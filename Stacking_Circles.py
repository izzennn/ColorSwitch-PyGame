...
base_x = (screen_x - radius * 2) // 2  # Center based on the screen width and circle diameter
star = pygame.image.load('star sprite.png')
color_changer = pygame.image.load('colorChange.png')
class Arc:
    def __init__(self, color, base_x, base_y, start_angle, stop_angle, widthh, big, small, smaller, first, first_circ_arc):
        self.first_circ_arc = first_circ_arc
        ...
        if self.small:
            self.rect = pygame.Rect(self.base_x + (radius - smallRadius), self.base_y + radius * 2, smallRadius * 2, smallRadius * 2)
        if self.smaller:
            self.rect = pygame.Rect(self.base_x + (radius - smallerRadius), self.base_y + radius * 2 + smallRadius * 2, smallerRadius * 2,
                            smallerRadius * 2)
    ...
    def display(self, screen):
        if self.alive:
            ...
            if self.first_arc and self.color_alive and self.first_circ_arc:
    ...
    def updateRect(self):
        if self.big:
            self.rect = pygame.Rect(self.base_x, self.base_y, radius * 2, radius * 2)
        if self.small:
            self.rect = pygame.Rect(self.base_x + (radius - smallRadius), self.base_y + radius * 2, smallRadius * 2,
                                    smallRadius * 2)
        if self.smaller:
            self.rect = pygame.Rect(self.base_x + (radius - smallerRadius), self.base_y + radius * 2 + smallRadius * 2,
                                    smallerRadius * 2, smallerRadius * 2)

        self.center = self.rect.center
        if not self.star_die and self.first_arc:
            self.image_rect.center = self.center
        if self.first_arc and self.color_alive and self.first_circ_arc:
            self.color_changer_image_rect.center = (self.center[0], (self.center[1] - radius) + self.total_obstacle_length() + 100)
    ...
    def total_obstacle_length(self):
        return radius * 2 + smallRadius * 2 + smallerRadius * 2


def create_Stacking_Circles(base_y):
    base_x = (screen_x - radius * 2) // 2
    base_y = base_y

    circle = [Arc(RED, base_x, base_y, math.radians(0), math.radians(90), 20, True, False, False, True, True),
              Arc(GREEN, base_x, base_y, math.radians(90), math.radians(180), 20, True, False, False, False, False),
              Arc(BLUE, base_x, base_y, math.radians(180), math.radians(270), 20, True, False, False, False, False),
              Arc(YELLOW, base_x, base_y, math.radians(270), math.radians(360), 20, True, False, False, False, False)]

    smallCircle = [Arc(RED, base_x, base_y, math.radians(0), math.radians(90), 20, False, True, False, True, False),
                   Arc(GREEN, base_x, base_y, math.radians(90), math.radians(180), 20, False, True, False, False, False),
                   Arc(BLUE, base_x, base_y, math.radians(180), math.radians(270), 20, False, True, False, False, False),
                   Arc(YELLOW, base_x, base_y, math.radians(270), math.radians(360), 20, False, True, False, False, False)]

    smallerCircle = [Arc(RED, base_x, base_y, math.radians(0), math.radians(90), 20, False, False, True, True, False),
                     Arc(GREEN, base_x, base_y, math.radians(90), math.radians(180), 20, False, False, True, False, False),
                     Arc(BLUE, base_x, base_y, math.radians(180), math.radians(270), 20, False, False, True, False, False),
                     Arc(YELLOW, base_x, base_y, math.radians(270), math.radians(360), 20, False, False, True, False, False)]
